<!--
  Version française du profil GitHub de Gavin-Qiao (Mohan Qiao).
  Édition anglaise (affichée sur le profil) : README.md
-->

<div align="center">
  <img
    src="./assets/banners/neo-01-grid.gif"
    alt="Mohan Qiao — bandeau néo-plasticisme"
    width="100%"
  />
</div>

<p align="center">
  <picture>
    <source
      media="(prefers-color-scheme: dark)"
      srcset="https://readme-typing-svg.demolab.com?font=IBM+Plex+Sans&weight=600&size=20&duration=3200&pause=900&color=E6EDF3&center=true&vCenter=true&multiline=false&width=700&height=36&lines=Ing%C3%A9nieur+IA+%26+agents+LLM;Doctorant+%40+Universit%C3%A9+Concordia;Fondateur+%40+Telotia;Directeur+TI+%40+SDPS"
    />
    <source
      media="(prefers-color-scheme: light)"
      srcset="https://readme-typing-svg.demolab.com?font=IBM+Plex+Sans&weight=600&size=20&duration=3200&pause=900&color=1F2328&center=true&vCenter=true&multiline=false&width=700&height=36&lines=Ing%C3%A9nieur+IA+%26+agents+LLM;Doctorant+%40+Universit%C3%A9+Concordia;Fondateur+%40+Telotia;Directeur+TI+%40+SDPS"
    />
    <img
      src="https://readme-typing-svg.demolab.com?font=IBM+Plex+Sans&weight=600&size=20&duration=3200&pause=900&color=E30613&center=true&vCenter=true&multiline=false&width=700&height=36&lines=Ing%C3%A9nieur+IA+%26+agents+LLM;Doctorant+%40+Universit%C3%A9+Concordia;Fondateur+%40+Telotia;Directeur+TI+%40+SDPS"
      alt="Rôles : ingénieur IA · doctorant Concordia · fondateur Telotia · directeur TI SDPS"
    />
  </picture>
</p>

<div align="center">

[**EN**](./README.md) · **FR**

**Systèmes multi-agents fiables et gouvernés**

Candidat au doctorat · Génie de l'information et des systèmes · Université Concordia  
Fondateur, [Telotia](https://telotia.com) · Montréal · bilingue EN / FR

[![Site](https://img.shields.io/badge/telotia.com-000000?style=flat-square&logo=safari&logoColor=white)](https://telotia.com)
[![SDPS](https://img.shields.io/badge/sdpsnet.org-0055A4?style=flat-square&logo=cloudflare&logoColor=white)](https://sdpsnet.org)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-E30613?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/mohan-qiao)
[![Courriel](https://img.shields.io/badge/mohan.qiao@mail.concordia.ca-000000?style=flat-square&logo=gmail&logoColor=FFCC00)](mailto:mohan.qiao@mail.concordia.ca)
[![GitHub](https://img.shields.io/badge/Gavin--Qiao-000000?style=flat-square&logo=github&logoColor=white)](https://github.com/Gavin-Qiao)

<br/>

> *Il est toujours, partout et pour quiconque, un tort de croire quoi que ce soit sur la base d'une preuve insuffisante.*  
> — W. K. Clifford, *The Ethics of Belief* (1877)

Je construis des systèmes qui traitent les affirmations comme des affirmations tant que la preuve n'a pas tranché.

</div>

---

## Projet phare — [sdpsnet.org](https://sdpsnet.org)

Principal artisan de la plateforme web de la Society for Design and Process Science — produit, pile technique, récupération des données, mise en production et exploitation au quotidien.

| | |
| --- | --- |
| **Site de la société** | [sdpsnet.org](https://sdpsnet.org) — adhésion, revue, histoire, direction |
| **Congrès** | [2026.sdpsnet.org](https://2026.sdpsnet.org) — SDPS 2026 · Louvain · 23–26 août 2026 |
| **Pile** | Astro · TypeScript · Cloudflare (Workers / D1 / R2) · paiements · courriel transactionnel |
| **Périmètre** | Inscription, soumission d'articles, facturation, gating de release, tests automatisés |

Récupération des données de la société après une compromission grave du serveur précédent, puis reconstruction et exploitation de la pile en production — y compris la facturation des services (Cloudflare, Resend).

---

## Sur quoi je travaille

Je conçois des **systèmes multi-agents LLM gouvernés** : orchestration, évaluation, garde-fous et provenance de qualité audit — de la recherche de preuves formellement vérifiée (Lean 4) aux plateformes de production pour processus réglementés.

| Axe | En pratique |
| --- | --- |
| **Agents fiables** | Orchestration multi-agents avec rôles explicites, modes de défaillance et critères de clôture |
| **Preuve et assurance** | Reçus à source préservée, moniteurs et échelles de claims qui tiennent sous examen |
| **Évaluation humaine et modèle** | Évaluation de la créativité interculturelle, notation calibrée, évaluation interprétable |

Actuellement : doctorat à Concordia, construction de Telotia (lancement public en septembre), et livraison de SDPS comme ci-dessus.

---

## Travaux sélectionnés

### Telotia — fondateur · arrivée en septembre
Logiciel de **validation affirmation → preuve** : un rapport et un corpus de preuves en lesquels vous avez confiance ; décomposition en affirmations atomiques ; pour chacune, les preuves à l'appui ou en contradiction — ou l'absence de preuve — avec un reçu citable et un verdict gradué. Ce n'est pas un chatbot et ne remplace pas la signature de l'expert ; ça s'assure que rien d'important n'a filé avant qu'un examinateur sceptique voie le dossier.

Premier produit : un banc de travail CAPA / enquêtes qualité pour les équipes qualité en dispositifs médicaux. Surface publique en **septembre**.  
→ [telotia.com](https://telotia.com)

### Faits saillants de recherche
- **Sécurité aéronautique** (collab. Collins Aerospace) — pipeline savoir → assurance : artéfacts étiquetés par autorité, moniteurs temporels, traces de falsification, contrats de sécurité RL  
- **Gauging-Ψ** — clustering exploratoire sans nombre de classes prédéfini (MST récursif + graphes de tolérance adaptatifs) ; 124 jeux de données étiquetés  
- **OCDE PISA 2025 pensée créative** — co-responsable, groupe anglais PISA Canada (collab. UBC) ; système d'évaluation PRISM + notation LLM de la pensée divergente  

---

## Pile technique

```text
IA / agents     Python · Lean 4 · RAG · évaluation et garde-fous · neuro-symbolique · RL
Langages        TypeScript · Rust · Elixir · SQL · C/C++
Plateformes     Cloudflare (Workers / D1 / R2) · Astro · Bun · FastAPI · Docker
Méthodes        Gouvernance de la preuve · vérification formelle · plan d'expérience
```

---

## Ailleurs sur ce profil

| Dépôt | Description |
| --- | --- |
| [**vserve**](https://github.com/Gavin-Qiao/vserve) | CLI pour gérer l'inférence vLLM sur stations GPU — téléchargement, réglage, service, ventilation |
| [**organon**](https://github.com/Gavin-Qiao/organon) | Outillage recherche / systèmes |
| [**concordia-style**](https://github.com/Gavin-Qiao/concordia-style) | Fondations du design system Concordia (React + Tailwind v4) |

Ce dépôt héberge aussi le **CV en code** — fichiers de contenu typés, une feuille de style print-grade, éditions EN/FR, export PDF reproductible.

---

## Parcours

| | |
| --- | --- |
| **Ph. D.** Génie de l'information et des systèmes | Université Concordia · 2024– · 4,30/4,30 · accéléré |
| **M. Sc. A.** Génie des systèmes qualité | Université Concordia · 2023–2024 · bourse de mérite |
| **B. Sc.** Mathématiques et informatique | Université McGill · majeure conjointe · 2019–2022 |
| **Service** | Directeur TI, Society of Design and Process Science (SDPS) — [sdpsnet.org](https://sdpsnet.org) |

Aussi : assistant d'enseignement (INSE 6411), stages de recherche (télédétection UAV, sécurité cyber-physique IoT), et co-auteur d'un travail sur la flexibilité cognitive interculturelle à partir de la résolution créative PISA (en révision).

---

<div align="center">

**Ouvert à la collaboration** sur la fiabilité des agents, les systèmes d'IA gouvernés par la preuve, et la recherche qui tient hors de la démo.

[Site](https://telotia.com) · [SDPS](https://sdpsnet.org) · [LinkedIn](https://linkedin.com/in/mohan-qiao) · [Courriel](mailto:mohan.qiao@mail.concordia.ca) · [GitHub](https://github.com/Gavin-Qiao) · [English](./README.md)

</div>
